...
#heteroscedasticidad 1 White o de Breusch-Pagan test
prueba_white<- bptest(fit)
print(prueba_white)
#Obtenemos una p = 0.0075<0.05
#Rechazamos H0 con significancia 0.05


#heteroscdasticidad 2 Goldfeld Quant
prueba_gq <- gqtest(fit)
print(prueba_gq)
...
X1_exp <- exp(X1)
X3_exp <- exp(X3)
fit<-lm(y~X1_exp+X3_exp)
print(fit)
anova(fit)
summary(fit)

#heteroscedasticidad 1 White
prueba_white<- bptest(fit)
print(prueba_white)
#Obtenemos una p = 0.0075<0.05
#Rechazamos H0 con significancia 0.05
...
#La media del error es distinta de cero con confianza del 95%
...
prueba_white<- bptest(modelo_glm)
print(prueba_white)
