 ...
 * Alumno de escuela con arreglos
 * 16/04/2018
 */
public class Alumno {
	...
	public boolean altaCalificacion(String materia, double calificacion){
		...
		if (this.numeroMaterias<MAX && !yaCurso(materia)){
	...
	public boolean yaCurso(String materia){
		int pos = 0;
		boolean resp = false;
		
		while(pos<numeroMaterias && !materias[pos].equals(materia))
			pos++;
		if(pos!=numeroMaterias)
			resp = true;
		return resp;
	}
	
	public double calificacion (String materia){
		int pos = 0;
		double resp = -1;
